"""Aggregate MA(2) b0 Gaussian NPE and Flow NPE results across seeds.

Searches for results in res/ma2_b0/.
"""
import os
import math
import numpy as np

BASE_DIRS = ["res/ma2_b0"]
...
# --- Main output ---
print("=" * 80)
print("MA(2) b0: KL DIVERGENCE — Gaussian NPE vs Flow NPE  (mean (std))")
print("=" * 80)
print_metric_table("kl.txt", fmt=".3f")

print("=" * 80)
print("MA(2) b0: MMD — Gaussian NPE vs Flow NPE  (mean (std))")
print("=" * 80)
print_metric_table("mmd.txt", fmt=".4f")

print("=" * 80)
print("MA(2) b0: COVERAGE — Gaussian NPE (mean 80%/90%/95% across seeds)")
print("=" * 80)
params = ["t1", "t2"]
for prefix, method in [("gaussian_npe", "Gaussian NPE"), ("npe", "Flow NPE")]:
    print(f"\n  {method}:")
    for n_obs in n_obs_list:
        for label, fn in zip(labels, sim_fns):
            n_sims = fn(n_obs)
